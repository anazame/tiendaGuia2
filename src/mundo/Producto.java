...
public class Producto {
    ...
    /**
     * Cantidad de unidades m�nima que debe haber en bodega para poder hacer un
     * pedido.
     */
     ...
     * <b> post: </b> El tipo, nombre, valor unitario, cantidad en bodega, cantidad
     * m�nima y ruta imagen fueron inicializados con los valores dado por par�metro.
     *
     ...
     * @param pCantidadMinima Cantidad m�nima que debe haber en bodega.
     *                        pCantidadMinima >= 0.
     * @param pRutaImagen     Ruta de la imagen del producto. pRutaImagen != null &&
     *                        pRutaImagen != "".
     */
    public Producto(String pTipo, String pNombre, double pValorUnitario, int pCantidadBodega, int pCantidadMinima,
            String pRutaImagen) {
    ...
    public void cambiarValorUnitario(double nuevoValorUnitario) {
        // TODO: Cambiar el valor unitario del
    ...
    public double calcularPrecioFinal() {
        ...
        return (this.darValorUnitario() + (this.darValorUnitario() * this.darIVA()));
    }

    /**
     * Retorna el IVA correspondiente al producto.
     * Tenga en cuenta que en esta tienda se manejan 3 tipos de productos:
     * supermercado, droguería y papelería. El IVA de cada tipo de producto es
     * diferente, así:
     * los productos de tipo supermercado tienen un IVA del 4%,
     * los productos del tipo droguería tienen un IVA del 12%
     * los productos de tipo papelería tienen un IVA del 16%.
     *
     * @return IVA.
     */
    public double darIVA() {
        ...
        switch (this.darTipo().toLowerCase()) {
            case "papeleria":
                iva = 0.16;
                break;
            case "supermercado":
     ...
     * @return Cantidad que realmente fue vendida, seg�n la disponibilidad en la
     *         bodega.
     */
    public int vender(int pCantidad) {
        int cantidadVendida = 0;
        // TODO: Vender la cantidad dada del producto.
        if (this.darCantidadBodega() < pCantidad) {
            cantidadVendida = this.darCantidadBodega();
            this.cambiarCantidadBodega(0);
        } else {
            cantidadVendida = pCantidad;
            this.cambiarCantidadBodega((this.darCantidadBodega() - pCantidad));
        }

        this.cambiardarCantidadUnidadesVendidas(this.darCantidadUnidadesVendidas() + cantidadVendida);
     ...
     *
     * @return True si la cantidad en la bodega es menor que la m�nima, false en
     *         caso contrario.
     */
    public boolean puedeAbastecer() {
        // TODO: Retornar true o false si se puede abastecer o no
        return (this.darCantidadBodega() < this.darCantidadMinima()) ? true : false;
    }
}